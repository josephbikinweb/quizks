...
function reveal() {
  ...
  const elementBeginning = document.getElementById("modal");
  elementBeginning.innerHTML = "";

  console.log(elementArray);
  console.log(elementArray.length);
  if (elementArray.lenght > 0) {
  ...
  for (let i = 0; i < savedAnswer.length; i++) {
    ...
    questionModal.classList.add("questionModal");
    answerModalRight.classList.add("answerModalRight");
    answerModalYours.classList.add("answerModalYours");

    const nodeQuestionModal = document.createTextNode(
      `${i + 1}. ${savedQuestion[i]} `
    );
    const nodeAnswerModalRight = document.createTextNode(correctsAnswer[i]);
    const nodeAnswerModalYours = document.createTextNode(
      `Jawaban anda : ${savedAnswer[i]} ---> Salah`
    );

    // if (savedAnswer[i] == correctsAnswer[i]) {
    //   const answerModalYours = document.createElement("h4");
    //   const nodeAnswerModalYours = document.createTextNode(
    //     `Jawaban anda : ${savedAnswer[i]} -> Benar`
    //   );
